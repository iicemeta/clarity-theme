...
export const compatibilityContract = [
	...
	},
	{
		group: 'core',
		feature: 'stats',
		input: 'stats.includePaths = posts/%',
		expected: '/api/stats 输出文章数、字数、分类与标签统计',
		command: 'pnpm test:compatibility && pnpm test:consumer',
		status: 'automated',
		coverage: ['compat:F-stats', 'consumer:generate-default'],
	},
	{
		group: 'core',
		feature: 'permalink',
	...
	{
		group: 'config',
		feature: 'useRandomPermalink=true',
		input: 'consumer 分支配置 article.useRandomPermalink=true',
		expected: 'schema 接受并保留该开关；构建不受影响（随机 permalink 生成属于构建脚手架，Theme 运行时只透传该配置）',
		command: 'pnpm test:consumer',
		status: 'partial',
		coverage: ['consumer:exports-smoke', 'consumer:generate-features-off'],
	},
	{
		group: 'config',
	...
	},
	{
		...
		expected: 'routeRules 关闭预渲染，generate 产物中无 api/stats',
		command: 'pnpm test:consumer',
		status: 'automated',
	...
	},
	{
		...
		expected: 'routeRules 关闭预渲染（产物无 atom.xml），head 无 alternate 声明',
		command: 'pnpm test:consumer',
		status: 'automated',
		coverage: ['consumer:generate-features-off'],
	},
	{
		...
		expected: 'routeRules 关闭预渲染，generate 产物中无 subscriptions.opml',
		command: 'pnpm test:consumer',
		status: 'automated',
		coverage: ['consumer:generate-features-off'],
	},
