...
export const ssrCases = [
	...
	},
	{
		...
		patterns: [
			...
			[/\n\tconst nested = \{/, 'tab preserved (upstream mdc patch)'],
			[/\n\t\tdeep: /, 'nested tab preserved (upstream mdc patch)'],
	...
	},
	{
		...
		// 上游 anti-mirror 始终注入（硬编码黑名单）；已废弃的自定义黑名单不得出现
...
export const requiredContractFeatures = {
	...
	'config': [
		...
		'atom=false',
		'opml=false',
		'antiMirror upstream (always-on)',
		'antiMirror deprecated custom blacklist',
		'antiMirror injection contract',
...
export const compatibilityContract = [
	...
	{
		group: 'config',
		feature: 'antiMirror upstream (always-on)',
		input: '默认配置（playground 与 consumer 默认变体）',
		expected: '上游 anti-mirror 模块始终注入硬编码黑名单与站点 URL（base64）',
		command: 'pnpm test:compatibility && pnpm test:consumer',
		status: 'automated',
	...
	{
		group: 'config',
		feature: 'antiMirror deprecated custom blacklist',
		input: 'features.antiMirror={ blacklist: [\'mirror.example.com\'] }（已废弃配置）',
		expected: '自定义黑名单不再注入；上游硬编码黑名单与站点 URL 仍然注入',
		command: 'pnpm test:consumer',
		status: 'automated',
		coverage: ['consumer:generate-branches'],
	},
	{
		group: 'config',
		feature: 'antiMirror injection contract',
		input: 'dev 服务（CLARITY_COMPAT_SITE_URL 注入站点 URL）',
		expected: 'dev SSR 输出包含上游黑名单与站点 URL 的 btoa 编码，且不含废弃的自定义黑名单',
		command: 'pnpm test:compatibility',
		status: 'automated',
		coverage: ['compat:anti-mirror-injection'],
	},
	{
		group: 'config',
