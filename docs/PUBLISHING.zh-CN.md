...
本文档说明 Clarity Theme 的版本管理、验证与带 provenance 的 npm 发布流程，面向包维护者。运行时 Theme 与 `create-clarity-theme` 是两个独立版本化的 npm 包。
...
- `create-clarity-theme/package.json` 独立版本化，不要求与 `clarity-theme/package.json` 同步。
...
- 创建包发布使用 `create-v<版本号>`（例如 `create-v0.1.0-beta.1`），由 `publish-create.yml` 处理，不会与 Theme tag 冲突。
...
`create-clarity-theme` 需要另配一个 Trusted Publisher：仓库、registry 与
environment 相同，但 workflow filename 为 `publish-create.yml`。该创建包首次
发布前的一次性配置仍待维护者在 npm 网站完成。
...
- 门禁 `clarity-theme@0.1.1` 已于 2026-09-22T09:45:26Z 经 OIDC 工作流发布。
- `clarity-theme@0.1.2` 已于 2026-09-22T15:04:33Z 经 OIDC 工作流发布，是 registry 的 `latest`。
...
[`publish-create.yml`](../.github/workflows/publish-create.yml) 是
`create-clarity-theme` 的独立等价发布流水线。它会忽略 Theme release tag、
校验 `create-v${创建包版本}`、串行运行创建器 CLI 与两条真实 E2E、打包并审计
创建器 tarball，最后通过 OIDC provenance 发布。
...
`create-clarity-theme` 发布后，验证 registry 元数据与公开命令：
...
## 13. 发布 create-clarity-theme

创建包没有运行时依赖，并独立发布：

1. 保持 `templates/default/package.json` 中的 `clarity-theme` 指向真实已发布的 registry 范围（当前为 `^0.1.2`）。
2. 只有 CLI 行为或模板变化时才提升 `create-clarity-theme/package.json`，并补充其 `CHANGELOG.md` 条目。
3. 在确切的发布 commit 上串行运行 `pnpm test:create`、`pnpm test:create:e2e` 与 `pnpm test:create:tarball`。
4. 确认 npm 上 workflow 为 `publish-create.yml` 的 Trusted Publisher 已一次性配置完成。
5. 初始预发布时，在确切 commit 上打 `create-v0.1.0-beta.1`，推送 tag，并从它发布 GitHub Release。工作流会自动把预发布分配到 npm dist-tag `beta`。
6. 确认工作流发布的是已审计 tarball 且带 provenance，然后执行第 9 节的 registry 验证命令。确认创建器不再需要变更后，再从 reviewed beta 提升 `0.1.0`。
