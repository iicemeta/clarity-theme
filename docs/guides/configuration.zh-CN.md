...
> src/config/app.ts 及其测试。API 边界见[公共 API](../reference/api.zh-CN.md)；当前验证状态见[项目状态](../maintainers/project-status.zh-CN.md)。
...
- 可见性列标记：**✅ Client-visible**（该值会进入客户端 bundle，**禁止存放任何密钥 / token**）；
  **⚙️ 仅服务端**（由 Nitro 在运行时消费，不进客户端 bundle）；
  **🏗 构建期**（由模块在构建期消费，不进 appConfig，但可能出现在生成的页面 HTML 中）
