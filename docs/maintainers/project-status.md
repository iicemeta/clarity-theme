...
- A `clarity-config` module loads and validates `clarity.config.ts` (Zod-strict), bridges site config into appConfig/Nitro runtime config, wires SEO/robots/llms/route rules/head scripts, applies permalink and `/posts`-prefix Content hooks, and injects the optional anti-mirror script.
