...
- `clarity-config` 模块加载并校验 `clarity.config.ts`（Zod 严格模式），把站点配置桥接到 appConfig/Nitro runtime config，接线 SEO/robots/llms/route rules/head 脚本，应用固定链接与 `/posts` 前缀 Content hook，并注入可选的反镜像脚本。
