...
确切的 Git URL 不会在各文档中重复出现，以保持 README 与 manifest 作为权威的
展示/引用位置。主题纯度校验不再拦截 parity 门禁保留的上游公开示例内容
（交流群、站点日志、图标、反镜像黑名单、Atom generator 署名）；上游站点的
私密数据（统计 ID、Token、私有服务端点等）仍然会被拦截。
