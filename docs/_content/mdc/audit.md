...
1. **No component-level gap.** All 27 upstream content components exist in `src/components/content/` and are registered by the Layer `components` config. Differences found during the diff are adaptations only (relative imports, ProseCode dual-mode).
